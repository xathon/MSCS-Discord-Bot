...
for (let guild of env.guilds) {
  if (guild.worlds.length === 1) {
    //only one world, so no need for options
    commands_mod = commands //don't modify the original array
  } else {
    //more than one world, so add options object
    let worldSelector = {}
    worldSelector.autocomplete = undefined
    worldSelector.choices = []
    for (let world of guild.worlds) {
      worldSelector.choices.push({ name: world.name, value: world.name })
    }
  ...
  if (process.argv.slice(2)[0] !== 'dry') {
    ...
    rest
      .put(Routes.applicationGuildCommands(env.client_id, guild.guildID), {
        body: commands_mod
      })
      .then(() =>
        console.log(
          `Successfully registered guild commands for ${guild.guildName}.`
        )
      )
      .catch(console.error)
  } else {
