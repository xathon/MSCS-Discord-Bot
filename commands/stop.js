...
module.exports = {
  ...
  async execute (interaction) {
    ...
    try {
      ...
      for (let guild of guilds) {
        if (guild.worlds.contains(worldName)) {
          ...
          env.logger.verbose(
            `Stop command came from server ${guild.guildName} for world ${worldName}.`
          )
          name = worldName
          found = true
          break
      ...
      for (let guild of guilds) {
        if (interaction.guild.id === guild.guildID) {
          ...
          env.logger.verbose(
            `Stop command came from server ${guild.guildName} for world ${guild.worlds[0]}.`
          )
          name = guild.worlds[0]
          found = true
          break
    ...
    if (!found) {
      ...
      env.logger.warn(
        `${interaction.user.username} requested to stop a world from guild ${interaction.guild.name}, but no world was found for this guild!`
      )
