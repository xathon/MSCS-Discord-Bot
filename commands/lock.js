...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addBooleanOption(option => option.setName('lock').setDescription('True to lock, false to unlock').setRequired(true))
    ...
    async execute(interaction) {
        ...
        if (interaction.options.getBoolean('lock')) {
            ...
            return interaction.reply({ content: 'Locked the server!', ephemeral: true });

        } else {
            console.log("Unlocking server.");
            await env.keyv.set("locked", "false");
            env.client.user.setStatus('idle');
            return interaction.reply({ content: 'Unlocked the server!', ephemeral: true });
        }
